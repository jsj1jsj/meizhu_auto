...
class InitLogin(object):
    def __init__(self):
        ...
        self.changeHotelUrl = readCon.get_config_value('login','changeHotelUrl')
    ...
    def login(self):
        # 登录
        logger.info('----开始准备登录----')
        result = self.reqMethod.send_request('post',self.loginUrl,data=self.data)
        # 保存token
    ...
    def save_headers_to_file(self,result):
        ...
        pmsHeader.save_token(self.dataKey,self.tokenKey,self.tokenField,self.appId)
        # 保存fms_token
        fmsHeader = OperateHeader(result,confPath.FMS_HEADERS_PATH)
        fmsHeader.save_token(self.dataKey, self.tokenKey, self.tokenField, self.fmsId)
        # 保存mis_token
        misHeader = OperateHeader(result,confPath.MIS_HEADERS_PATH)
        misHeader.save_token(self.dataKey,self.tokenKey,self.tokenField,self.misId)
